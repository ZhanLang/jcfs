...
CString CChiToLetter::GetLetter(CStringA strText)
{
    TBYTE ucHigh, ucLow;
    int  nCode,j;
    CString strValue="";
    ...
    {
        if ( (BYTE)strText[i] < 0x80 )
		{
			strValue=strValue+ strText[i];
			if(m_LetterEnd) strValue=strValue+_T('\'');
            continue;
		}
        ucHigh = (BYTE)strText[i];
        ucLow  = (BYTE)strText[i+1];
...
CString CChiToLetter::FindLetter(int nCode)
{
	CString strValue;
	switch(nCode)
	{

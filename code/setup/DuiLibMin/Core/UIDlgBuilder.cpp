...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
	...
	if (pParent && _tcscmp(pParent->GetClass(), _T("Template")) == 0)
	{
		CTemplateUI* pXmlTmpl = dynamic_cast<CTemplateUI*>(pParent);
		if (pXmlTmpl)
		{
			pXmlTmpl->SetTemplateXml(pRoot->GetValueAll());
			return NULL;
		}
	}
...
//				else if( _tcscmp(pstrClass, DUI_CTR_DATETIME) == 0 )		  pControl = new CDateTimeUI;
				...
				//else if( _tcscmp(pstrClass, DUI_CTR_ICONVIEW) == 0 )		  pControl = new CIconViewUI;
				else if (_tcscmp(pstrClass, DUI_CTR_TEMPLATE) == 0)			  pControl = new CTemplateUI;
                break;
            case 9:
...
//				else if( _tcscmp(pstrClass, DUI_CTR_WEBBROWSER) == 0 )        pControl = new CWebBrowserUI;
			...
			case 11:
				...
				//if (_tcscmp(pstrClass, DUI_CTR_VIRTUALLIST) == 0)			  pControl = new CVirtualListUI;
				break;
            case 14:

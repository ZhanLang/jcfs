...
CControlUI* CPaintManagerUI::CreateTemplate(LPCTSTR lpszTemp, IDialogBuilderCallback* pCallback /*= NULL*/, CPaintManagerUI* pManager /*= NULL*/, CControlUI* pParent /*= NULL*/)
{
	CTemplateUI* pTemp = static_cast<CTemplateUI*>(this->FindControl(lpszTemp));
	if (pTemp)
	{
		CDuiString strXml;
		strXml.Format(_T("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n%s\r\n<Window>"), pTemp->GetTemplateXml().GetData());
		DuiLib::CDialogBuilder builder;		
		return builder.Create(strXml.GetData(), (UINT)0,  pCallback, pManager, pParent);
	}

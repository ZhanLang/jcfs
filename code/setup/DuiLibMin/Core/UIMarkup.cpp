...
CMarkupNode* CMarkupNode::GetChildPtr()
{
...
CDuiString CMarkupNode::GetValueAll() const
{
	CDuiString strRet;

	CDuiString strTmp = m_pOwner->m_strTmpl.Mid(m_pOwner->m_pElements[m_iPos].iStart);
	CDuiString strName = _T("</");
	strName += GetName();
	int n = strTmp.Find(strName, 1);
	if (n == -1){
		return strRet;
	}
	int n2 = strTmp.Find(_T('>'));
	if (n2 == -1)
	{
		return strRet;
	}

	strRet = strTmp.Mid(n2+1, n - n2 - 1);
	return strRet;
}

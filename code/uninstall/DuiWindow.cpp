...
// DUI_END_MESSAGE_MAP()
#define WMMSG_SET_PROG (WM_USER + 1001)
#define WMMSG_SET_PAGE (WM_USER + 1002)
...
DuiLib::CDuiString CDuiWindow::GetSkinFolder()
{
	if ( m_pSetupWizard->IsDebug() )
	{
		 TCHAR szTemp[MAX_PATH] = { 0 };
...
{
	SendMessage(WMMSG_SET_PROG, nProgress);
}
...
{
	CDuiString fileName = m_pSetupWizard->GetProgramPosition();
...
VOID CDuiWindow::OnClick(TNotifyUI& msg)
{
	...
	if (_tcsicmp(_T("addto_disktop_button"), msg.pSender->GetName()) == 0)
	{
		CCheckBoxUI* pCheckBox = static_cast<CCheckBoxUI*>(m_pm.FindControl(_T("addto_disktop_checkbox")));
		RASSERTV(pCheckBox);
		pCheckBox->SetCheck(!pCheckBox->GetCheck());
	}

	if (_tcsicmp(_T("set_mainpage_button"), msg.pSender->GetName()) == 0)
	{
		CCheckBoxUI* pCheckBox = static_cast<CCheckBoxUI*>(m_pm.FindControl(_T("set_mainpage_checkbox")));
		RASSERTV(pCheckBox);
		pCheckBox->SetCheck(!pCheckBox->GetCheck());
	}

	if (_tcsicmp(_T("addto_task_button"), msg.pSender->GetName()) == 0)
	{
		CCheckBoxUI* pCheckBox = static_cast<CCheckBoxUI*>(m_pm.FindControl(_T("addto_taskbar_checkbox")));
		RASSERTV(pCheckBox);
		pCheckBox->SetCheck(!pCheckBox->GetCheck());	
	}

	if (_tcsicmp(_T("btn_remove"), msg.pSender->GetName()) == 0)
	{
